...
const ItCompaniesCard = ({companyLogo}) => {
    ...
    return (
        <div
          className="bg-white h-60 rounded-lg shadow-md p-4 transition duration-300 transform hover:-translate-y-2 hover:shadow-lg relative flex justify-center items-center"
          onMouseEnter={() => setIsHovering(true)}
          onMouseLeave={() => setIsHovering(false)}
        >
          <img className='h-full' src={companyLogo} alt="" />
          {isHovering && (
